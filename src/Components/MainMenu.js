...
const MainMenu = ({ cards }) => {
    ...
    return (
        <div className="flex flex-col m-2 w-8/12 mx-auto p-2">
            {cards && cards.map((menuSection) => {
                if (menuSection?.card.card?.["@type"] === "type.googleapis.com/swiggy.presentation.food.v2.ItemCategory") {
                    const title = menuSection?.card?.card?.title;
                    const { itemCards } = menuSection?.card?.card;
                    ...
                    return (
                        <div key={title} className="">
                            <ResMenuHeader title={title} size={itemCards.length} flag={true} showMenu={showMenu[title]} toggleMenu={() => toggleMenu(title)} />
                            {itemCards && showMenu[title] && itemCards.map((item) => (
                                <Recommendation menu={item.card.info} key={item.card.info.id} />
                            ))}
                        </div>
                    );
                }
                // Handle NestedItemCategory similarly...
            })}
        </div>
    );
};
